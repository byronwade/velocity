...
- [x] **chat-drag-pin** — Drag any chat message (yours or a coworker's reply) onto a panel to pin it as work at the exact drop position, with auto-intent and auto-assignment. _(shipped 2026-07-16)_
- [x] **agent-handoffs** — Coworkers communicate with each other: real changed files are handed to QA in chat with a real streamed response, and a reply that names a teammate pulls that teammate into the conversation (bounded chains, real replies only). _(shipped 2026-07-16)_
